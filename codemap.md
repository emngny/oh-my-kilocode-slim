...
`oh-my-kilocode-slim` is an KiloCode plugin that implements a specialist-agent operating model on top of the host runtime. Its core responsibilities include:

- Defining chief and specialist agent factories with permission policies
...
This codemap covers the plugin repository itself and excludes the nested `kilo/` upstream checkout.
...
| `scripts/generate-schema.ts` | Generates `oh-my-kilocode-slim.schema.json` from the Zod config schema. |
...
| `src/agents/` | Agent factory layer for chief and specialists, including prompt/model overrides, display-name normalization, MCP assignment, and permission shaping. | [View Map](src/agents/codemap.md) |
...
| `src/hooks/phase-reminder/` | Message-transform reminder enforcing chief workflow phases. | [View Map](src/hooks/phase-reminder/codemap.md) |
...
| `src/skills/` | Bundled install-time KiloCode skills shipped as static payloads. | [View Map](src/skills/codemap.md) |
...
| `src/skills/clonedeps/` | Workflow-only dependency source mirroring skill that routes discovery/ref resolution through librarian and direct chief git operations. | [View Map](src/skills/clonedeps/codemap.md) |
...
2. **Interactive request handling**
   - The chief prompt drives routing decisions.
   - Tool calls resolve through `src/tools/` or built-in KiloCode tools.
...
3. **Delegated execution**
   - Native KiloCode background tasks are parsed from `task` output and injected completion messages and tracked in the shared background job board.
   - `src/hooks/task-session-manager/` updates job-board state, resolves short aliases, and injects background/reusable job context into the chief prompt.
...
4. **Install/release path**
   - `src/cli/` configures host KiloCode instances.
