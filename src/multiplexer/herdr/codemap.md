...
- `spawnPane` resolves the parent pane via `HERDR_PANE_ID` or `--current`, splits it in the configured direction, renames the new pane to the agent description, and runs `kilo attach` via `herdr pane run`. Pane IDs are extracted by parsing JSON CLI output from `herdr pane split`.
...
- `spawnPane(sessionId, description, serverUrl, directory)`:
  ...
  - run `kilo attach <url> --session <sessionId> --dir <directory>` via `herdr pane run <pane>`
...
- UI attach command semantics are identical to tmux in argument shape: `kilo attach <url> --session <sessionId> --dir <directory>`, so delegated sessions remain config-agnostic across backends.
