...
## Responsibility
Manages the cloning and management of read-only dependency source repositories into `.slim/clonedeps/repos/` for offline inspection. This skill provides a workflow (not a command wrapper) that guides the chief and `@librarian` through cloning dependency sources so agents can inspect library internals without requiring network access.

## Design
- **Workflow skill, not a command wrapper**: No helper scripts or TypeScript utility functions. The chief owns the decision-making; `@librarian` recommends sources; the chief performs filesystem/git operations directly.
...
3. **Verify and confirm**: Chief verifies refs with `git ls-remote`, avoids unsafe URLs, presents plan to user, and gets approval before cloning.
4. **Clone sources**: Chief runs git commands directly. Creates one folder per source under `.slim/clonedeps/repos/<safe-repo-name>/`. Prefers shallow clones, pinned tags, HTTPS URLs.
