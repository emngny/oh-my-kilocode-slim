...
`clonedeps` is a bundled KiloCode workflow skill for cloning a small set of
important dependency source repositories into a local ignored workspace so agents
can read library internals.

It is chief-owned. The chief delegates source discovery and URL/ref
...
and cloning are repo-specific enough that the chief/librarian workflow is
...
metadata. After cloning, the chief should add or update a concise
