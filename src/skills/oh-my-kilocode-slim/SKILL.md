...
description: Configure and improve oh-my-kilocode-slim for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# oh-my-kilocode-slim Configuration Skill

You help users configure, customize, and safely improve their
oh-my-kilocode-slim setup.
...
- `.kilo/` or `~/.config/kilo/` plugin/agent configuration
- agent models, variants, presets, or provider routing
- chief delegation behavior or specialist-agent prompts
- custom agents under `agents.<name>`
- custom agent `prompt` and `chiefPrompt` blocks
...
oh-my-kilocode-slim is configured through a plugin config file, usually:
...
```text
~/.config/kilo/oh-my-kilocode-slim.jsonc
```
...
| Path | Use |
|---|---|
| `~/.config/kilo/kilo.json` | KiloCode core config: plugin registration and providers |
| `~/.config/kilo/oh-my-kilocode-slim.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/kilo/oh-my-kilocode-slim.json` | User plugin config generated by installer |
| `<project>/.kilo/oh-my-kilocode-slim.json` | Project-local plugin overrides checked before user config |
| `~/.config/kilo/oh-my-kilocode-slim/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/kilo/oh-my-kilocode-slim/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/kilo/oh-my-kilocode-slim/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/kilo/oh-my-kilocode-slim/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
| `~/.config/kilo/skills/<skill-name>/SKILL.md` | Installed skill prompt payload |

Built-in agent prompt file names are exact agent names:

- `chief.md` / `chief_append.md`
...
1. If a `preset` is active, check
   `~/.config/kilo/oh-my-kilocode-slim/{preset}/` first.
2. Fall back to `~/.config/kilo/oh-my-kilocode-slim/`.
...
- **Improve quality**: use stronger models for `chief`, `oracle`, or
...
- **Tune prompts**: override or extend an agent's prompt for local workflow
  preferences.
- **Guide delegation**: add `chiefPrompt` for custom agents so the
  Chief knows when to call them and when not to.

Important schema boundary:

- Built-in agents (`chief`, `oracle`, `librarian`, `explorer`,
  `designer`, `fixer`, `observer`, `council`) can set models, variants, skills,
  MCPs, options, and display names in config.
- Built-in agent `prompt` and `chiefPrompt` fields are **not** supported
  in `oh-my-kilocode-slim.json[c]`; use markdown prompt override files instead.
- Unknown keys under top-level `agents` are custom agents. Custom agents may use
  `prompt` and `chiefPrompt` directly in config.
...
```text
~/.config/kilo/oh-my-kilocode-slim/chief_append.md
```

or for only the `openai` preset:

```text
~/.config/kilo/oh-my-kilocode-slim/openai/chief_append.md
```

Example content:

```markdown
## Local Chief Preference
...
```text
~/.config/kilo/oh-my-kilocode-slim/chief.md
```
...
5. **Tell the user about restart requirements.**
   - KiloCode may need a restart for config, prompt, agent, skill, MCP, or plugin
     changes to take effect.
   - Phrase it as: "This should apply on the next KiloCode run; restart KiloCode
...
1. **Inspect current setup**
   - Read the existing oh-my-kilocode-slim config file.
   - Identify active `preset` and relevant agent blocks.
   - Check whether the user has local project `.kilo/` overrides.
...
6. **Explain activation**
   - Tell the user whether the change applies immediately or on next KiloCode
     run/restart.
...
```jsonc
{
  "agents": {
    "api-reviewer": {
      ...
      "chiefPrompt": "Delegate to @api-reviewer for API contract changes, public SDK changes, backwards-compatibility questions, or migration-risk review. Do not use it for routine implementation.",
      "skills": [],
      "mcps": []
    }
  }
...
- a specific job;
- clear trigger conditions in `chiefPrompt`;
...
- The Chief repeatedly delegates too much or too little for this user's
  workflow.
...
### Make Chief more conservative with background writers

Proposal:

```text
I can add a narrow Chief prompt rule to avoid parallel writer agents in
the same directory unless their file ownership is explicit. Target:
~/.config/kilo/oh-my-kilocode-slim/chief_append.md. Apply it?
```
...
`api-reviewer` with no skills or MCPs and an `chiefPrompt` that routes
public API changes to it. Apply to your oh-my-kilocode-slim config?
```
...
```text
Updated the config. This should apply on the next KiloCode run; restart
KiloCode if you need it immediately.
```
...
- [ ] Did you mention KiloCode restart/next-run behavior?
