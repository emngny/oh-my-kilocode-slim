...
## Responsibility
Orchestrates phase reminder injection into user messages for the chief agent, ensuring workflow guidance is appended without mutating the original message content or affecting UI display.

## Design

### Core Abstraction
- **Hook Factory**: `createPhaseReminderHook()` returns an KiloCode experimental chat message transformer hook
- **Non-Mutating Strategy**: Appends phase reminder as a separate message part rather than modifying user-authored text
- **Targeted Injection**: Only processes messages from the chief agent
...
1. **Hook Invocation**: KiloCode calls the `experimental.chat.messages.transform` hook before sending messages to API
2. **Message Analysis**: Iterates backward through messages to find the last user message
3. **Agent Validation**: Confirms the message is from the chief agent
...
### Consumers
- **KiloCode**: Registers the hook via plugin initialization
- **Chief Agent**: Receives phase reminders in messages
...
- **Scope**: Only affects chief agent messages
