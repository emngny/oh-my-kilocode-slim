...
## Responsibility
Implements KiloCode lifecycle hooks that transform, process, and manage chat messages and attachments during the plugin's message pipeline. These hooks are invoked by KiloCode's `experimental.chat.messages.transform` API to modify message content before it reaches models or after responses are generated.

## Design

### Core Architecture
- **Factory Pattern**: Each hook is created via a factory function (e.g., `createApplyPatchHook()`, `createAutoUpdateCheckerHook()`) that returns a hook function matching the KiloCode hook signature.
...
### Hook Categories
1. **Attachment Processing**: `processImageAttachments` - Extracts image data URLs from messages, saves them to `.kilo/images/` directory, and replaces image parts with text references containing file paths.
...
```
1. KiloCode receives chat messages
...
7. Final messages are returned to KiloCode
```
...
2. For each user message with images:
   ...
   c. Save image to `.kilo/images/[sessionID]/` directory
...
3. Hooks are registered with KiloCode via `experimental.chat.messages.transform`
4. KiloCode invokes hooks during message lifecycle
```

## Integration

### Consumers
- **Main Plugin**: `src/index.ts` - registers hooks with KiloCode during plugin initialization
- **KiloCode Runtime**: Invokes hooks during `experimental.chat.messages.transform` API calls

### Dependencies
- **KiloCode SDK**: Type definitions for `MessageWithParts`, `MessageInfo`, and hook signatures
...
- **Workspace Directory**: Images are saved to `.kilo/images/` within the project workspace
