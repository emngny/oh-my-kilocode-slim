...
You can enable the companion by adding a `companion` section to your setting configuration file (`~/.config/kilo/oh-my-kilocode-slim.json` or `.kilo/oh-my-kilocode-slim.json`):

```jsonc
{
  "companion": {
    "enabled": true,
    "binaryPath": "/path/to/oh-my-kilocode-slim-companion",
...
- **`companion.debug`**: set to `true` to enable verbose native companion debug
  ...
  `$XDG_DATA_HOME/kilo/log/` or `~/.local/share/kilo/log/`.
...
```bash
bunx oh-my-kilocode-slim install --companion=yes
```
...
Wayland app-id/title: `oh-my-kilocode-slim-companion`.
...
```text
$XDG_DATA_HOME/kilo/storage/oh-my-kilocode-slim/bin/oh-my-kilocode-slim-companion
```

If `XDG_DATA_HOME` is unset, this resolves to:

```text
~/.local/share/kilo/storage/oh-my-kilocode-slim/bin/oh-my-kilocode-slim-companion
```
...
so KiloCode startup is not blocked by a slow network. Plugin auto-update also
...
on the next KiloCode restart. The updater uses a lock to prevent concurrent
KiloCode processes from replacing the same binary, and stale locks from crashed
...
```bash
bunx oh-my-kilocode-slim@beta install --companion=yes
```
