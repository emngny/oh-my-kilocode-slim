...
Bundled skills are installed by the `oh-my-kilocode-slim` installer and safely
...
| [`codemap`](#codemap) | Repository codemap generation | `chief` |
| [`clonedeps`](#clonedeps) | Local dependency source cloning | `chief` |
| [`deepwork`](#deepwork) | Heavy/complex coding sessions workflow | `chief` |
| [`reflect`](#reflect) | Review repeated work and suggest reusable workflow improvements | `chief` |
| [`worktrees`](#worktrees) | Safe Git worktree lane management | `chief` |
| [`release-smoke-test`](#release-smoke-test) | Packed release-candidate and bugfix smoke validation | `chief` |
| [`oh-my-kilocode-slim`](#oh-my-kilocode-slim) | Plugin configuration and self-improvement guidance | `chief` |
...
By default, this skill is assigned to `oracle`, which owns code review, maintainability review, and simplification guidance. The `chief` should route simplification requests to `oracle` instead of handling them as a top-level specialty itself.
...
`codemap` empowers the Chief to build and maintain a deep architectural understanding of any codebase. Instead of reading thousands of lines of code on every task, agents refer to hierarchical `codemap.md` files describing the *why* and *how* of each directory.

**How to use:** Ask the Chief to `run codemap`. It automatically detects whether to initialize a new map or update an existing one.
...
`clonedeps` helps the Chief clone a small, approved set of dependency
source repositories into `.slim/clonedeps/repos/` so KiloCode can inspect library
internals while keeping cloned code out of git.

The skill is assigned to `chief`. The chief may ask `@librarian`
...
are handled by the chief/librarian workflow so the skill works across
languages and repository types.

Before planning, the chief checks `.slim/clonedeps.json` and reuses
...
`deepwork` is an chief-only workflow skill for managing deep architectural work, multi-phase implementations, and complex refactoring. It provides a structured approach with mandatory review gates while maintaining flexibility in planning.
...
**How it works:**
1. Chief creates a session artifact at `.slim/deepwork/<task>.md`
...
- KiloCode todo lists for progress tracking
- Flexible structure - chief adapts format to task needs
...
`reflect` is an chief-only workflow skill for reviewing recent work,
...
`worktrees` is an chief-only skill for managing Git worktrees as safe,
isolated coding lanes. Instead of polluting your current branch or juggling stash
state, the Chief can set up lanes under `.slim/worktrees/<slug>/` and
track them in `.slim/worktrees.json`.

Other agents can be delegated tasks inside the worktree lane, but the Chief coordinates the lifecycle, validation, and final integration.
...
`oh-my-kilocode-slim` is an chief-only skill that teaches agents how to
configure the plugin itself: model presets, custom agents, agent prompts,
`chiefPrompt` delegation hints, skills, MCP permissions, optional agents,
and related KiloCode config files.

It is installed by default with the bundled skills and is available to the
Chief through the default `skills: ["*"]` configuration.

The skill also tells the Chief to notice repeatable workflow friction and
...
that KiloCode may need a restart for config, prompt, agent, skill, MCP, or plugin
changes to take effect.

Typical requests:

```text
Tune my oh-my-kilocode-slim models for lower cost.
Add a custom API reviewer agent.
Make the Chief more conservative about parallel writer agents.
...
```text
This should apply on the next KiloCode run; restart KiloCode if you need it immediately.
```
...
`release-smoke-test` is an chief-only skill for proving a release branch
works as an installed package artifact. It builds and packs the candidate,
installs the tarball into a throwaway app, runs KiloCode with a sanitized
...
smokes such as KiloCode 1.17.11 malformed message transform regressions.
...
Control which skills each agent can use in `~/.config/kilo/oh-my-kilocode-slim.json` (or `.jsonc`):
