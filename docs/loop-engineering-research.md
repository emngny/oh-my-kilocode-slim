...
## Tool Comparison: Claude Code vs Codex vs KiloCode

### Automations / Scheduling

| Feature | Claude Code | Codex | KiloCode |
|---------|-------------|-------|----------|
...
**Verdict:** Claude Code and Codex both ship full automation primitives. KiloCode has the loop engine fully designed in spec but not yet implemented.

### Worktrees (Isolation)

| Feature | Claude Code | Codex | KiloCode |
|---------|-------------|-------|----------|
| Git worktree support | Yes (`--worktree`) | Yes (built-in per thread) | Yes (chief skill) |
...
**Verdict:** Claude Code and Codex have worktrees as runtime primitives integrated with their loop engines. KiloCode has worktrees as an chief skill with apply-patch support, but no programmatic runtime integration yet.
...
**Verdict:** All three have mature skill systems. KiloCode's is notably rich with 7 bundled skills, per-agent permission control, and automatic sync on plugin updates.
...
**Verdict:** All three support MCP. KiloCode stands out with 3 built-in MCPs (websearch, context7, gh_grep) and a sophisticated per-agent permission system.
...
| Maker/checker split | Yes | Yes | Yes (chief dispatches, specialists execute) |
...
| Parallel dispatch | Yes | Yes | Yes (explicit in chief prompt) |

**Verdict:** All three have sub-agent support. KiloCode's is the most structured with 9 specialized agents, a formal Background Job Board, session reuse, depth tracking, and a dedicated chief that never implements directly.
...
**Verdict:** Claude Code and Codex have working loop primitives. KiloCode's loop engine is fully designed with a 3-layer architecture (Chief -> LoopEngine -> Specialists) but not yet implemented.
...
## KiloCode's Loop Engineering Status
...
| **Worktrees** | Skill-only | Chief skill, apply-patch hook support |
...
KiloCode has 3 of 5 building blocks fully wired (skills, connectors, sub-agents), worktrees as a skill, and automations + loop engine as unimplemented specs. The sub-agent infrastructure is the strongest piece - the Background Job Board, session reuse, and depth tracking are more structured than what Claude Code or Codex expose.

The missing piece is the outer loop itself: the scheduler that runs on a timer, spawns work, and keeps going without human intervention. Once that lands (Phase 1-2 of the spec), KiloCode will have a complete loop engineering stack.
...
6. **KiloCode is 60% there.** Skills, connectors, and sub-agents are mature. The loop engine and automations are designed but unimplemented. The sub-agent infrastructure is the strongest piece of the stack.
