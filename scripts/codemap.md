...
- `generate-schema.ts`
  ...
  - Writes `oh-my-kilocode-slim.schema.json` with explicit `$schema`, `title`, and plugin description.
...
- `verify-kilo-host-smoke.ts`
  - Builds temporary KiloCode environment (bin from `bun add kilo-ai`), mounts the plugin tarball,
    launches `kilo serve`, and probes `http://127.0.0.1:<port>/global/health`.
...
- `bun run verify:host-smoke` runs `verify-kilo-host-smoke.ts`: pack tarball -> boot isolated host -> wait for health -> verify no plugin-load errors.
