...
oh-my-kilocode-slim 是一个用于 KiloCode 的智能体编排插件。它内置了一支专业的智能体团队，可以在同一个编排者（Chief）下，完成侦察代码库、查询最新文档、审查架构、处理 UI 工作以及执行范围明确的实现任务。
...
> “任务管理轻松从 5/10 提升到了 8-9/10。Chief 会派出
> Fixer 和 Explorer，而我仍然可以在同一个会话里继续和 Chief
> 对话与规划。现在整个体验顺滑多了。”
>
> \- `vipor_idk`
...
> “我很喜欢 omo-slim，已经无法想象不用它来运行 kilo。
...
安装程序会把本地仓库路径加入 `~/.config/kilo/kilo.json` 的
`plugin` 数组，因此 KiloCode 会从该文件夹加载插件。之后要更新：
...
安装程序会同时生成 OpenAI 和 KiloCode Go 预设，默认启用 OpenAI。

> [!TIP]
> 根据自己的工作流自由微调模型和智能体。默认预设只是起点；本插件的目标是为用户提供深度灵活性和可定制性。

要在安装期间启用 KiloCode Go，请运行 `bunx oh-my-kilocode-slim@latest install --preset=kilo-go`，或在安装后修改 `~/.config/kilo/oh-my-kilocode-slim.json` 中的默认预设名称。
...
1. **登录您想要使用的模型服务商账户（如果您还没有登录的话）**：
   ...
   kilo auth login
   ```
2. **刷新并列出 KiloCode 可以调用的模型**：

   ```bash
   kilo models --refresh
   ```
3. **打开您的插件配置文件**，路径为 `~/.config/kilo/oh-my-kilocode-slim.json`

4. **为您要分配的每个智能体更新模型配置**

> [!TIP]
> **建议**了解后台编排的工作原理。**[编排者提示词 (Chief prompt)](https://github.com/alvinunreal/oh-my-kilocode-slim/blob/master/src/agents/chief.ts#L28)** 包含调度规则、专家路由逻辑，以及何时应把工作分配给后台智能体的阈值。您始终可以通过以下方式手动委派任务：`@智能体名称 <任务内容>`

> [!TIP]
> 由于后台智能体现在是默认工作流，**强烈建议**启用并配置 **[Multiplexer Integration](docs/multiplexer-integration.md)**。它会自动在专用的 Tmux、Zellij 或 Herdr 窗格中打开每个智能体，让您在 Chief 继续协调会话时，实时跟进各个专家智能体的工作。

默认生成的配置包含 `openai` 和 `kilo-go` 两个预设：
...
V2 将 oh-my-kilocode-slim 变成了以调度器为核心的多智能体工作流系统。Chief 专注于规划、委派、结果整合与验证，而专家智能体在各自的工作通道中完成任务。

- **[后台智能体](#后台智能体)** - Chief 现在会把专家作为后台任务派发，跟踪任务/会话 ID，等待完成事件，并在继续之前整合结果。
...
- **[oh-my-kilocode-slim Skill](#oh-my-kilocode-slim-skill)** - 随包提供的配置技能，可安全调优模型、提示词、自定义智能体、MCP 访问、预设和插件行为。

#### 后台智能体

V2 将后台专家作为默认心智模型：Chief 规划工作图，启动合适的智能体，避免重叠的写入所有权，并在处理终端任务结果后再继续行动。
...
Reflect 帮助 Chief 从重复出现的工作流摩擦中学习。它会回顾近期工作和现有资产，然后建议最小且有用的改进：skill、自定义智能体、命令、配置规则、提示词规则、MCP 权限变更或项目 playbook。如果证据不足，它应建议什么都不创建。
...
Worktrees 将 Git worktree 作为安全、隔离的编码通道管理，默认位于 `.slim/worktrees/<slug>/`。Chief 负责这些通道的生命周期，在 `.slim/worktrees.json` 中跟踪状态，在通道内调度专家智能体，并在修改 Git 状态前要求明确确认。

安全协议见 **[Skills](docs/skills.md#worktrees)**。

#### oh-my-kilocode-slim Skill

内置的 `oh-my-kilocode-slim` skill 可帮助 Chief 配置和改进插件本身。可用于模型调优、自定义智能体、提示词覆盖、skill/MCP 权限、预设、可选智能体、后台编排以及反复出现的工作流摩擦。
...
示例和安全规则见 **[Skills](docs/skills.md#oh-my-kilocode-slim)**。
...
<table>
  <tr>
    ...
    <td width="70%" valign="top">
      当第一个代码库在自身的复杂性下崩溃时，Chief 诞生了。神明与凡人都无法承担责任--因此 Chief 从虚无中显现，从混沌中建立秩序。它确定实现任何目标的最优路径，平衡速度、质量和成本。它引导整个团队，为每项任务召唤合适的专家，并通过委派任务以获得最佳成果。
    </td>
  </tr>
  <tr>
    <td colspan="2">
  ...
  <tr>
    <td colspan="2">
      <b>提示词源码：</b> <a href="src/agents/chief.ts"><code>chief.ts</code></a>
    </td>
  </tr>
  <tr>
    <td colspan="2">
  ...
  <tr>
    <td colspan="2">
      <b>模型选用指南：</b> 选择您的默认、最强的全能型编程模型。Chief 既是核心编程智能体，又是委派者，因此它需要强大的实现能力、出色的判断力和可靠的指令遵循度。
    </td>
  </tr>
</table>
...
> [!NOTE]
> **为什么 Chief 不经常自动调用 Council？** 这是刻意设计的。Council 会同时运行多个模型，由于这通常是系统中成本最高的路径，因此自动委派逻辑非常严格。在实际使用中，Council 旨在供您手动调用，例如：<code>@council 比较这两种架构</code>。
...
<table>
  ...
  <tr>
    <td colspan="2">
      <b>模型选用指南：</b> 选择一个快速、可靠的编程模型来执行常规且范围明确的开发工作。Fixer 通常从 Chief 接收具体的计划或受限的指令，非常适合高效执行诸如编写测试、更新测试和直接的代码更改等任务。
    </td>
  </tr>
</table>
...
> [!NOTE]
> **为什么要独立出一个智能体？** 如果您的 Chief 模型不是多模态模型，可以启用 Observer 来处理图像、屏幕截图、PDF 以及其他视觉文件。Observer 默认是禁用的，它在无需您更改核心推理模型的情况下，为 Chief 赋予了专用的多模态读取能力。只需在您的配置中设置 `disabled_agents: []` 并指定一个 `observer` 模型即可。自带的 `kilo-go` 安装预设会自动执行此操作，因为其 GLM Chief  不是多模态模型。
...
**只读视觉分析** -- 解读图像、屏幕截图、PDF 和图表。将结构化的观察结果返回给 Chief，而无需将原始文件字节加载到主上下文窗口中。

- 图像、屏幕截图、图表 → `read` 工具（原生图像支持）
- PDF 和二进制文档 → `read` 工具（文本 + 结构提取）
- **默认禁用** -- 通过设置 `"disabled_agents": []` 和配置具有视觉能力的模型来启用；若使用 `--preset=kilo-go` 预设安装，将自动使用 `kilo-go/kimi-k2.6` 启用它。
...
| **[自定义智能体](docs/configuration.md#custom-agents)** | 使用自定义提示词、模型、MCP 访问和 Chief 委派规则定义自己的专家 |
...
| **[后台编排](docs/background-orchestration.md)** | 围绕原生后台子智能体构建的调度器优先 Chief 模型 |
| **[维护者指南](docs/maintainers.md)** | issue 分流规则、标签含义、支持路由和仓库维护工作流 |
| **[Skills](docs/skills.md)** | `simplify`、`codemap`、`clonedeps`、`deepwork`、`reflect`、`worktrees` 和 `oh-my-kilocode-slim` 等捆绑技能 |
...
| **[KiloCode Go 预设](docs/kilo-go-preset.md)** | 安装程序生成的捆绑 `kilo-go` 预设 |
